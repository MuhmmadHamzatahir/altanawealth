...
function addFundClassIfRequired() {
    ...
    var result = pageToClassMap.filter(x => x.pageName === thisPage)[0];

    if result.fundClass.length != 0 {
      document.getElementsByTagName("BODY")[0].classList.add(result.fundClass);
    } else {
    }
};
